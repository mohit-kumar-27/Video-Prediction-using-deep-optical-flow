...
from test_time_adaptation.loss import unsupervised_loss
import cv2
import scipy.interpolate


def warp_unidirectional_flow(image, flow):
    """
    Warp given image frame with flow
    :param image: Image (Frame)
    :param flow: Optical flow to be warped
    """
...
class FlowControllerTTA:
    ...
    def test_time_adaptation(self, image1, image2, num_iter=1, lr=1e-4):
        (data, target) = self.preprocess_frames(image1, image2)
        if args.cuda:
            data, target = [d.cuda() for d in data], [t.cuda() for t in target]
        data, target = [Variable(d) for d in data], [Variable(t) for t in target]
