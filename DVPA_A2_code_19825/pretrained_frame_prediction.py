...
def deep_optical_flow(model_path, firstImage, secondImage, image_ind, dataset):
    ...
    if not os.path.exists(f"./results/pretrained/optical_flow/{dataset}/"):
        os.makedirs(f"./results/pretrained/optical_flow/{dataset}/")
...
def corridor_frame_prediction(model_path="./flownet2/pretrained_models/FlowNet2_checkpoint.pth.tar"):
    ...
    for ind in range(0, len(images) - 2, 1):
        ...
        forward_flow, If = deep_optical_flow(model_path, firstImage, secondImage, ind, "corridor")
...
def sphere_frame_prediction(model_path="./flownet2/pretrained_models/FlowNet2_checkpoint.pth.tar"):
    ...
    for ind in range(0, len(images) - 2, 1):
        ...
        forward_flow, If = deep_optical_flow(model_path, firstImage, secondImage, ind, "sphere")
...
def predict_frame(secondImage, forward_flow, If, image_ind, dataset):
    ...
    if not os.path.exists(f"./results/pretrained/predicted_frames/{dataset}/"):
        os.makedirs(f"./results/pretrained/predicted_frames/{dataset}/")
    cv2.imwrite(f"./results/pretrained/predicted_frames/{dataset}/forward_prediction_{image_ind+2}.png",
    warped_image,)

    return cv2.imread(f"./results/pretrained/predicted_frames/{dataset}/forward_prediction_{image_ind+2}.png",flags=cv2.IMREAD_GRAYSCALE)
